...
async function get_disruption({ res }) {
/**
 ...
 */
  ...
  const simple_disruption = await call_tfl.get_disruption(false)
  ...
  if (is_there_disruption) {
    // if there's disruption, then call the "detailed" version of the disruption data
    logger.debug('There is disruption, calling the detailed version of the disruption data')
    const detailed_disruption = await call_tfl.get_disruption(true)
